# encoding: utf-8
"Single classifier using neural networks"
...
if train is not None:
    ...
    allIngredients, trainLabels = dataReader(train)
...
#encode the cuisine labels to numbers
le = preprocessing.LabelEncoder()
le.fit(trainLabels)
trainLabels = le.transform(trainLabels)

#scale the training data to appropriate form for neural networks
scaler = StandardScaler()
scaler.fit(matrix)
matrix = scaler.transform(matrix)
ingredientMatrix = scaler.transform(ingredientMatrix)

#fit the training data to neural network model
mlp = MLPClassifier(hidden_layer_sizes=(100,), max_iter=20, alpha=1e-4,
                    solver='sgd', verbose=False, tol=1e-4, random_state=None,
                    learning_rate_init=.001)

mlp.fit(matrix, trainLabels)

#predict the result and transform it back to string
result = mlp.predict(ingredientMatrix)
